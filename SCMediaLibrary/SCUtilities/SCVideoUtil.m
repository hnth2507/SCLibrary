...
+ (UIImage *)imageThumbnailFromVideo:(AVAsset*)asset atTime:(CMTime)time
{
...
+ (UIImage *)imageThumbnailFromURL:(NSURL *) url
{
    ...
    return [self imageThumbnailFromVideo:asset atTime:CMTimeMake( SC_VIDEO_FPS, SC_VIDEO_FPS)];
}

+ (SCVideoOrientation)getVideoOrientation:(AVAsset *)asset
{
    SCVideoOrientation result = SCVideoOrientationNone;
    ...
    if([tracks    count] > 0) {
        ...
        {
            result = SCVideoOrientationPotrait;
        }
        // PortraitUpsideDown
        if(t.a == 0 && t.b == -1.0 && t.c == 1.0 && t.d == 0)  {
            result = SCVideoOrientationUpsideDown;
        }
        // LandscapeRight
        if(t.a == 1.0 && t.b == 0 && t.c == 0 && t.d == 1.0)
        {
            result = SCVideoOrientationLanscapeRight;
        }
        // LandscapeLeft
        if(t.a == -1.0 && t.b == 0 && t.c == 0 && t.d == -1.0)
        {
            result = SCVideoOrientationLanscapeLeft;
        }
    }
    return result;
}

+ (BOOL)checkVideoWithVineType:(SCVideoComposition*)video
{
    if(CGSizeEqualToSize(video.naturalSize, SC_VIDEO_VINE_SIZE) && CMTimeGetSeconds(video.duration) <= SC_VIDEO_VINE_DURATION)
        return YES;
    
    return NO;
}

+ (BOOL)checkVideoWithInstagramType:(SCVideoComposition*)video
{
    if(CGSizeEqualToSize(video.naturalSize, SC_VIDEO_VINE_SIZE) && CMTimeGetSeconds(video.duration) <= SC_VIDEO_INSTAGRAM_DURATION)
        return YES;
...
+ (float)videoDurationFrom:(NSURL*)url
{
    SCVideoComposition *video = [SCVideoComposition videoItemWithURL:url];
    if(video)
        return CMTimeGetSeconds(video.assetDuration);
