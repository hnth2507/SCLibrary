...
+ (NSArray*)URLPathsFromDocumentWithPrefix:(NSString*)prefix extension:(NSString*)extension
{
    NSMutableArray *result= [[NSMutableArray alloc]init];
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString *documentsDirectory = [paths objectAtIndex:0];
    ...
    while ((filename = [direnum nextObject] ))
    {
        ...
        {
            [result addObject:[SCFileManager URLFromDocumentWithName:filename]];
        }
    }
...
+ (NSArray*)URLPathsFromBundleWithPrefix:(NSString*)prefix extension:(NSString*)extension
{
    NSMutableArray *result= [[NSMutableArray alloc]init];
    NSArray *files = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:[[NSBundle mainBundle] bundlePath] error:NULL];
    for (NSString *fileName in files) {
        if ([fileName.pathExtension isEqualToString:extension] && [fileName hasPrefix:prefix])
        {
            [result addObject:[SCFileManager URLFromBundleWithName:fileName]];
        }
    }
...
+ (NSArray*)namesFromBundleWithPrefix:(NSString*)prefix extension:(NSString *)extension
{
    NSArray *files = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:[[NSBundle mainBundle] bundlePath] error:NULL];
...
+ (NSArray*)resourceNamesFromBundleWithPrefix:(NSString*)prefix
{
    NSMutableArray *result= [[NSMutableArray alloc]init];
    NSString *bundleRoot = [[NSBundle mainBundle] bundlePath];
    NSFileManager *manager = [NSFileManager defaultManager];
    ...
    while ((filename = [direnum nextObject] ))
    {
        if ([filename hasPrefix:prefix] && ![filename hasSuffix:@"@2x"])
        {
            [result addObject:filename];
...
+ (NSURL *)createIncreaseNameFromTempWith:(NSString *)name andtype:(NSString *)type
{
	...
	do {
		...
		NSString *numberString = count > 0 ? [NSString stringWithFormat:@"-%lu", (unsigned long)count] : @"-0";
