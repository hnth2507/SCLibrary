...
@synthesize mediaExtension = _mediaExtension;
...
- (id)init
{
    ...
    if(self)
    {
        ...
        _isComposing    = NO;
        _iSOverWrite    = NO;
        _mediaExportQuality = AVAssetExportPresetHighestQuality;
        _mediaFormat    = SC_MEDIA_TYPE_MPEG4;
        _mediaExtension = SC_MP4;
        _FPS = CMTimeMake(1, SC_VIDEO_OUTPUT_FPS);
        _name = SC_OUTPUT_VIDEO;
        self.slideIndex = 0;
        self.totalDuration = CMTimeMake(0, SC_VIDEO_FPS);
...
- (void)addSlideComposition:(SCSlideComposition*)slide
{
    if(self.slides)
    {
        if(!slide.name.length)
            slide.name = [NSString stringWithFormat:@"slide-%d-%@", (int)self.slides.count,[SCHelper getCurrentDateTimeInString]];
...
- (void)addTransitionAfterSlideIndex:(int)index transition:(SCTransitionComposition*)transition
{
    ...
    if(index <= self.slides.count - 2 && self.slides.count >= 2)
    {
        ...
        NSLog(@"[Notice] Transition index is wrong (%d) because there is only %d slide",index,(int)self.slides.count);
}
...
- (void)deleteSlideComposition:(SCSlideComposition *)slide
{
    if(self.slides.count > 0 && slide)
    {
        ...
        int index = (int)[self.slides indexOfObject:slide];
